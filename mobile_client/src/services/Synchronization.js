				...
				this._refreshToken().then(() => {
					...
					Promise.all([promiseSalesChannels, promiseCustomerTypes])
						.then( (values) => {
							...
							// Promise.all([promiseCustomers, promiseProducts, promiseSales, promiseProductMrps, promiseReminders, promiseReceipts])
							// 	.then(values => {
							// 		resolve(syncResult);
							// 	})

							// This will make sure they run synchronously
							[promiseCustomers, promiseProducts, promiseSales, promiseProductMrps, promiseReminders, promiseReceipts]
